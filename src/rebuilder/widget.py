...
from src.profiler.service import ProfilerThread
...
class RebuilderWidget(QtWidgets.QWidget, ThreadManager):
    ...
    def __init__(self, main_window,
        ...
        self.profiler: ProfilerThread | None = ProfilerThread(None)
    ...
    def translate_ui(self) -> None:
        ...
        if self.restricted_codec:
            self.ui.ac3.setText(self.tr("convert dts to ac3: Skip"))
        ...
        logger.info("Rebuilder translate applied")
    ...
    def set_mkv_ac3_tools_path(self) -> None:
        ...
        logger.info(f"Mkv/eac3to path set: mkvextract - {self.mkv_extract_path}, mvkmerge - {self.mkv_merge_path},"
                    f" eac3to - {self.ac3_converter}")
    ...
    @logger.catch
    def run_profiler(self, process_id: int) -> None:
        """Запуск потока профилирования"""
        if settings.get_profiler_status is True:
            self.profiler.process_id = process_id
            self.profiler.profiler_signal.connect(self.main_window.statusBar().showMessage)
            self.profiler.start()

    def showEvent(self, event) -> None:
        """Выполнение функций при открытии виджета"""
    ...
    def start_work(self) -> None:
        ...
        if self.restricted_codec == "AC-3":
            logger.info("Start works with AC-3")
            self.thread_manager.start(self.extract_ac3_track)
        elif self.restricted_codec == "E-AC-3":
            logger.info("Start works with E-AC-3")
            self.thread_manager.start(self.extract_eac3_track)
        else:
            logger.info("Start works with DTS")
    ...
    def convert_to_ac3(self):
        ...
        command = subprocess.Popen([f"{self.ac3_converter}", self.dts_path, self.ac3_path, f"-{self.bitrate}", f"-down6"],
    ...
    def build_new_mkv_with_eac3_codec(self):
        ...
        if self.subtitle_data:
            ...
            command = subprocess.Popen(f'{self.mkv_merge_path} -o {self.output_file} -A -S {self.source_path}'
                                       f' {sound_metadata} {self.eac3_path} {sub_command} {self.subtitle_path}',
                                       stdout=subprocess.PIPE, text=True, creationflags=subprocess.CREATE_NO_WINDOW)
            self.run_profiler(command.pid)
            for line in command.stdout:
            ...
            command = subprocess.Popen(f'{self.mkv_merge_path} -o {self.output_file} -A -S {self.source_path}'
                                       f' {sound_metadata} {self.eac3_path}',
                                       stdout=subprocess.PIPE, text=True, creationflags=subprocess.CREATE_NO_WINDOW)
            self.run_profiler(command.pid)
            for line in command.stdout:
                self.ui.mkv.setText(self.tr(f"rebuild mkv: {line}"))
    ...
    def build_new_mkv(self):
        ...
        if self.subtitle_data:
            sub_command = f'--language 0:{self.subtitle_lang} --track-name 0:"{self.subtitle_name}"'
            command = subprocess.Popen(f'{self.mkv_merge_path} -o {self.output_file} -A -S {self.source_path}'
                                       f' {sound_metadata} {self.ac3_path} {sub_command} {self.subtitle_path}',
                                       stdout=subprocess.PIPE, text=True, creationflags=subprocess.CREATE_NO_WINDOW)
            self.run_profiler(command.pid)
            for line in command.stdout:
                self.ui.mkv.setText(self.tr(f"rebuild mkv: {line}"))
            ...
            command = subprocess.Popen(f'{self.mkv_merge_path} -o {self.output_file} -A -S {self.source_path}'
                                       f' {sound_metadata} {self.ac3_path}',
                                       stdout=subprocess.PIPE, text=True, creationflags=subprocess.CREATE_NO_WINDOW)
            self.run_profiler(command.pid)
            for line in command.stdout:
                self.ui.mkv.setText(self.tr(f"rebuild mkv: {line}"))
